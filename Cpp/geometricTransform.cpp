...
{
	// Translation 중간 이후
	Ty *= -1;// bmp파일은 거꾸로 저장됨 -> Ty의 값이 반대방향 값으로 이동해야 영상이 원하는 방향으로 이동 함
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			if ((i + Ty < H && i + Ty >= 0) && (j + Tx < W && j + Tx >= 0))// 영상의 범위를 넘지 않는 경우
...
{
	// Scaling -> 1보다크면 확대, 1보다 작으면 축소
	int tmpX, tmpY;
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
			tmpX = (int)(j / SF_X);
			tmpY = (int)(i / SF_Y);
			if (tmpY < H && tmpX < W)// 바뀐 x, y의 값이 H,W범위 안에 있는지 확인
				Output[i * W + j] = Image[tmpY * W + tmpX];// 역방향사상
		}
	}
	//for (int i = 0; i < H; i++) {
	//	for (int j = 0; j < W; j++) {
	...
	//		// 이런 방식은 넓어진 공간에 원본 영상을 쪼개서 넣어주기 때문에 영상에 주기적인 hole이 생기게 된다
	//	}
	//}
}
...
{
	int tmpX, tmpY;
	double Radian = Angle * 3.141592 / 180.0;// Angle은 degree형태 어쨋든 30도 회전을 원하면 출력 영상기준으로 -30도인 부분의 영상을 가져오면 된다
	for (int i = 0; i < H; i++) {
		for (int j = 0; j < W; j++) {
